...
def writeBridges(rows,filename):
    with open(filename, 'w') as fixed_csvfile:
        fieldnames = [Constants.road,Constants.km,Constants.lrpType,Constants.lrpname,Constants.name,
            ...
            Constants.zone,Constants.circle,Constants.division,Constants.subDivision,Constants.lat,Constants.lon,Constants.estimatedLoc]
...
def writeBridgesFinal(source, destination):
    ...
    worksheet = workbook.add_worksheet('BMMS_overview')
    ...
    for index, row in enumerate(rows):
        ...
        worksheet.write_number('L{}'.format(index+1),int(float(row[Constants.constructionYear])) if row[Constants.constructionYear] != '' else -1)
        ...
        worksheet.write_string('T{}'.format(index+1),row[Constants.estimatedLoc])
